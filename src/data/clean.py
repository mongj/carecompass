...
def fetch_image(name, width, height):
    response = requests.get("https://places.googleapis.com/v1/" + name + "/media?key=AIzaSyDLbsEteD1XnMvDNRdXHOgGWlxHTSIT9I4&maxWidthPx=" + str(width) + "&skipHttpRedirect=true")
    res = response.json()
    return res
...
def parse_google_map_response(data):
    ...
    for place in data.get('places', []):
        ...
        for photo in place.get('photos', []):
            # fetch photo data
            apiData = fetch_image(photo.get('name'), photo.get('widthPx'), photo.get('heightPx'))
